...
#TODO maybe make show_lengend different for each subplot???

# Create a function to generate the 1D plot
function create_1D_plot!(p, data, label, min_val, max_val, title_font_size, axis_font_size, xlabel, ylabel, show_labels, show_ticks, show_legend)
    plot!(
        p,
        data, 
        label=label,
...
function animate_1D_plots(datasets, titles, t, layout, size_; 
    ...
    anim = @animate for i = 1:n_snaps
        ...
        for j = 1:n_dataset
            if const_ylim
                create_1D_plot!(p[subplot_indices[j]], datasets[j][:,i], labels[j], global_min[j], global_max[j], 
                                title_font_size, axis_font_size, xlabels[j], ylabels[j], show_labels, show_ticks,show_legend)
            else
                create_1D_plot!(p[subplot_indices[j]], datasets[j][:,i], labels[j], minimum(datasets[j][:,i]), maximum(datasets[j][:,i]), 
                                title_font_size, axis_font_size, xlabels[j], ylabels[j], show_labels, show_ticks,show_legend)
            end
        end
    end
...
#-------- create 1D subplots 
function plot_1D_subplots(datasets, titles, indices, layout, size_; 
    ...
    for j = 1:length(datasets)
        create_1D_plot!(p[subplot_indices[j]], datasets[j][:,indices[j]], labels[j], 
